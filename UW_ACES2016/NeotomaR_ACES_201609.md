...
# UW ACES - Starter datasets

As a starter exercise, we'll download the following datasets from the following sites: 

1) Spicer Lake.  Tower Lake is a moderate-resolution record (~centennial) with a well-expressed series of major vegetational change events corresponding to the last deglaciation, and a crash in Fagus americana abundances a few thousand years ago.  See Wang et al. (2015) Holocene.
```r
spicer_site <- get_site(sitename = 'Spicer Lake%')
#As of Aug 2016, only one site named 'Spicer Lake' in NeotomaDB. This line may need to be fixed if more 'Spicer Lakes' are uploaded to Neotoma.
spicer_meta_dataset<-get_dataset(spicer_site[1,])
spicer_data <- get_download(spicer_site)
print(spicer_data)
```2) Tower Lake.  Tower Lake is a high-resolution record (sub-centennial) with a well-expressed signal of the mid-Holocene Tsuga decline and paired fossil pollen and plant macrofossil records.  See Jackson et al. 2014 QSR.
```r
tower_site <- get_site(sitename = 'Tower Lake%')
#As of Aug 2016, only one site named 'Tower Lake' in NeotomaDB. This line may need to be fixed if more 'Tower Lakes' are uploaded to Neotoma.
tower_meta_dataset<-get_dataset(tower_site[1,])
tower_data <- get_download(tower_site)
##Tower Lake has four cores and eight datasets - one pollen and plant macrofossil dataset per core.  
print(tower_data)
```
3) Kettle Lake.  Kettle Lake is a very high-resolution record (~decadal) of drought variability and shifts in grassland plant community composition in the northern Great Plains.  See Grimm et al. 2011 QSR.  Dolomite minerals are indicators of wetter conditions, while aragonite and quartz minerals are indicators of drier conditions.  
```r
kettle_site <- get_site(sitename = 'Kettle Lake%')
#As of Aug 2016, only two sites named 'Kettle Lake' in NeotomaDB and we want the first one, 
#in North Dakota.  This line may need to be fixed if more 'Kettle Lakes' are uploaded to Neotoma.
kettle_meta_dataset<-get_dataset(kettle_site[1,])
kettle_data <- get_download(kettle_site)
print(kettle_data)
```
Recall that get_download returns an object of type downloadlist.  Each item in this list is a download object that corresponds to a dataset (pollen data, geochemical data, etc.).  Each download object is itself a list of lists and data frames that contain information about the site

```r
##A series of quick print commands to review the datasets at Kettle Lake and the pollen dataset in particular. 
##The pollen data is dataset 5 at Kettle Lake
print(kettle_data)
print(kettle_data[[5]])
head(kettle_data[[5]]$sample.meta)
head(kettle_data[[5]]$taxon.list)
head(kettle_data[[5]]$counts)
```
