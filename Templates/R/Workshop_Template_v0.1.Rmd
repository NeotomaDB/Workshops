...
This workshop is intended to ... More teaching materials can be found [here](http://www.neotomadb.org/education/category/higher_ed/)
...
tester <- system.time(Bacon("LAKEPINES", acc.mean = 10, thick = 50, depths.file = TRUE, suggest = FALSE, ask = FALSE))

```

Bacon itself doesn't return data back into R, so I'm just timing the function so we know how long it takes.  The last run of Lake O' Pines took `r tester[3]` seconds to run.  
...
head(read_bacon("LAKEPINES", add = FALSE))
...
lake_o_dl[[1]] <- read_bacon("LAKEPINES", add = TRUE, download = lake_o_dl[[1]], sections = 17)
...
So we can immediately see the difference a new model makes, and since we've set the new model as the default we can now carry that forward into subsequent analysis with the newly revised `lake_o_dl`.
